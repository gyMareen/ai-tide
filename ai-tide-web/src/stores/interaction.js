...
export const useInteractionStore = defineStore('interaction', () => {
    // 本地状态
    const likedContents = ref(new Set());
    const favoriteContents = ref(new Set());
    const userRatings = ref(new Map());
    ...
    // 提交评论
    async function submitComment(contentId, comment, parentId) {
        const token = getToken();
        if (!token) {
            throw new Error('请先登录');
        }
    ...
    async function fetchUserLikes() {
        ...
        try {
            ...
            if (data.code === 200 && data.data) {
                likedContents.value = new Set(data.data.map((item) => item.contentId));
            }
        }
        catch (e) {
    ...
    async function fetchUserFavorites() {
        ...
        try {
            ...
            const data = await response.json();
            if (data.code === 200 && data.data) {
                favoriteContents.value = new Set(data.data.map((item) => item.contentId));
            }
        }
        catch (e) {
